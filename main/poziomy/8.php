...
<div id="srodek">
    <h1>LEVEL 8</h1>
    <h1 id="dzialanie">(1/2 + 3/4) × 8 - 2 ÷ (1/4) + 5 = 
    ...
    if(isset($_SESSION['nazwa'])){
        if ($twojPoziom > $poziom) {
            echo '<button class="submit" id="potwierdz" onclick="sprawdzOdpBez8()">ALL DONE</button>';
            exit(); 
    }else{
        echo '<button class="submit" id="potwierdz" onclick="sprawdzOdp8()">ALL DONE</button>';
    }
}
else{
    echo '<button class="submit" id="potwierdz" onclick="sprawdzOdpBez8()">ALL DONE</button>';
